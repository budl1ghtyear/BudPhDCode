...
JMeasurementModelC::JMeasurementModelC(const MeanCovarianceC &lp, const MeanCovarianceC &sk, const ImageC<RealRGBValueC> &img, const IntT &wd)
{
	lip = lp.Copy();
	skin = sk.Copy();
	origimg = img.Copy();
...
ImageC<VectorC> JMeasurementModelC::GetNormRGVals(const ImageC<RealRGBValueC> &im)
{
...
RealT JMeasurementModelC::Measure(ParticleC &pt)
{
...
Array1dC<Index2dC> JMeasurementModelC::VectortoPoint(const VectorC &vec)
{
...
Array1dC<Point2dC> JMeasurementModelC::VectortoPointArray(const VectorC &vec)
{
...
ImageC<RealT> JMeasurementModelC::ComputeJImage(const ImageC<RealRGBValueC> &img, const MeanCovarianceC &mc)
{
...
ImageC<RealT> JMeasurementModelC::ComputeJImage(const ImageC<VectorC> &img, const MeanCovarianceC &mc)
{
	RealT logsigma = Log((RealT)mc.Covariance().Det());
	ImageC<RealT> out(img.Frame());
	out.Fill(0.0);
...
RealT JMeasurementModelC::GetJWeight(const Array1dC<Point2dC> &ar, ImageC<RealT> &mc, ImageC<RealT> &sk)
{
...
RealT JMeasurementModelC::GetJWeight(const Array1dC<Point2dC> &ar)
{
...
RealT JMeasurementModelC::ComputeJ(const MeanCovarianceC &mc, const VectorC &vec)
{
	return (-Log(mc.Covariance().Det()) - mc.MahalanobisDistance(vec));
}

//RealT JMeasurementModelC::GetGaussianValue(const RealT &sigma, const RealT &xval)
//{
