...
int main(int nargs,char **argv) 
{
	...
	while(!dinputstream.IsGetEOS())//is end of stream?
	{
		...
		try
		{
  		...
  		cout<<"Got Eyes"<<endl;
  		///////////////////////////
  		//Make a pair of training images for optical flow
  		//This code is not optimal for speed
  		//For some reason, OpticFlow method doesn't like two images that don't start at 0,0
  		IndexRange2dC roi = botfacehalf.Expand(20) ; 
  		//cout<<"ROI = "<<roi<<endl;
		ImageC<RealT> img1(prev_img.Frame());img1.Fill(0.0);
		//cout<<"Img1 Frame = "<<img1.Frame();
		for(Array2dIter2C<RealT, RealRGBValueC> it(img1, prev_img,roi); it; it++)
		{
			it.Data1() = it.Data2().Y();
		}
		//if(!Save("@X: Image1",img1)) cerr<<"Could not save OF image"<<endl;
		//ImageC<RealT> img2(roi, 0.0);
		ImageC<RealT> img2(img.Frame());img2.Fill(0.0);
		for(Array2dIter2C<RealT, RealRGBValueC> it(img2, img,roi); it; it++)
		{
			it.Data1() = it.Data2().Y();
		}
		//if(!Save("@X: Image2",img2)) cerr<<"Could not save OF image"<<endl;
		//ImageC<RealT> im1(img1,roi),im2(img2,roi);
		PairC<ImageC<RealT> > realimgs(img1, img2);
		//INSERT TRACKING CODE IN HERE
		//Instantiate Propagation Model
		//cout<<"Propagation"<<endl;
		HSOpticalFlowPropagationModelC ofprop(realimgs,(RealT)noise);
		...
		Array1dC<Point2dC> spl_pts2 = bspl.RenderCurve(30); //Draw B-Spline Curve
		...
		prev_img = img.Copy(); //Need to reassign the idea of the "previous image" for the next frame
		//IntT s;
		//cin>>s;
	}	
	return 0;
