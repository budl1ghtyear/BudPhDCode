...
{	
	...
	DirectoryC pcadir = opt.String("p","/vol/vssp/lip-tracking/StatisticalShapeModelsOfLips/C1TrainingModels/PCA/","Directory containing pca data");
	...
	//LOAD PCA DATA
	PCAProjectionC pca((StringC)pcadir,numcomp);
	...
	for(DLIterC<StringC> it(files); it; it++)
	{
		...
		{
			if(it2->Size() == 11)
				pcapts.Append(pca.PerformPCAProjection(*it2).Copy());
		}
