...
{	
	...
	DirectoryC features = opt.String("d","/vol/vssp/lip-tracking/BiometricLipFiles/VanillaShapeFeatures/ShapeFeatures","Directory containing vanilla features");
	...
	DListC<StringC> feat_files = GetFeatureFiles(features, ext, feattype);
	...
	for(DLIterC<StringC> it(feat_files); it; it++)
	{
		FilenameC orgfile = (*it);
		FilenameC outfile = (StringC)output + "/" + (StringC)(orgfile.NameComponent());
		SampleC<VectorC> diffs = ComputeFrameDifferences(ReadPointsFromVectorFile(orgfile));
		WriteVectorsToTSVFile(diffs,outfile,orgfile);
	}
	return 0;
}
