...
int main(int nargs,char **argv) 
{
	...
	StringC outfilestr = (StringC)resdir+first_file.BaseNameComponent()+"_HPCA"+StringC(numpts)+StringC(search)+".txt";
	...
	while(din.GetAt(i,im))
	{
		...
		if(i == 1)
		{
			...
			PCAStateVectorC initpts(pca_dir, spl.GetControlPoints(),numcomponents);
		...
		else
		{
			...
			ImageRectangleC myrect = ObtainMouthLocalisationWindow(first_pts,1);
			...
			ChiHistMeasurementModelC meas(im,myrect);
