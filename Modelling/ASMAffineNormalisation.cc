...
//! rcsid="$Id: ASMAffineNormalisation.cc 7358 2009-11-04 11:46:22Z omn-crida $"
//! lib=RavlAAM
//! file="Ravl/CompVision/ActiveAppearanceModels/ASMAffineNormalisation.cc"
...
#include "ASMAffineNormalisation.hh"
  ...
  //: Default constructor.
  ASMAffineNormalisationBodyC::ASMAffineNormalisationBodyC()
  {}

  //: Load from bin stream.
  ASMAffineNormalisationBodyC::ASMAffineNormalisationBodyC(BinIStreamC &is)
    : ASMNormalisationBodyC(is)
  {}

  //: Load from stream.
  ASMAffineNormalisationBodyC::ASMAffineNormalisationBodyC(istream &is)
    : ASMNormalisationBodyC(is)
  {}

  //: Save to binary stream 'out'.
  bool ASMAffineNormalisationBodyC::Save(BinOStreamC &out) const {
    return ASMNormalisationBodyC::Save(out);
  }

  //: Save to stream 'out'.
  bool ASMAffineNormalisationBodyC::Save(ostream &out) const {
    return ASMNormalisationBodyC::Save(out);
  }

  //: Compute mean control points for the list of appearance provided.
  bool ASMAffineNormalisationBodyC::ComputeMean(SampleStreamC<AAMAppearanceC> &sample) {
    if (sample.Size() == 0)
  ...
  bool ASMAffineNormalisationBodyC::RawParameters(const AAMAppearanceC &inst,VectorC &fixedParams,VectorC &freeParams) const {
  ...
  void ASMAffineNormalisationBodyC::RawProject(const VectorC &fixedParams,const VectorC &freeParams,SArray1dC<Point2dC> &out) const {
  ...
  IntT ASMAffineNormalisationBodyC::NoFixedParameters() const {
    return 6;
  }

  RAVL_INITVIRTUALCONSTRUCTOR_FULL(ASMAffineNormalisationBodyC,ASMAffineNormalisationC,ASMNormalisationC);
